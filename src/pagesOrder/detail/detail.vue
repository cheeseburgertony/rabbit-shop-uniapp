...
const onCancelOrder = async () => {
  if (!reason.value) return uni.showToast({ icon: 'error', title: '请选择取消理由' })
  // 发送请求取消订单
  const res = await putMemberOrderByIdCancelAPI(query.id, reason.value)
  order.value = res.result
